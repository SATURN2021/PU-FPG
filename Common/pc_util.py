...
def extract_knn_patch(queries, pc, k, normals):
    """
    queries [M, C]
    pc [P, C]
    """
    ...
    k_normal_patches = np.take(normals, knn_idx, axis=0)
    return k_patches, k_normal_patches
...
def downsample_points(pts, K, use_FPS=True):
    # if num_pts > 8K use farthest sampling
    # else use random sampling
    if pts.shape[0] >= 2 * K and use_FPS:
...
def read_ply_with_color(file, count=None):
    ...
    if 'red' in loaded['vertex'].data.dtype.names:
        colors = np.vstack(
            [loaded['vertex'].data['red'], loaded['vertex'].data['green'], loaded['vertex'].data['blue']])
