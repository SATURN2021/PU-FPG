...
def up_unit(x=None,
    ...
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        if upsample.lower() == 'nodeshuffle':
            x = nodeshuffle(x, DGG, spat_feat, topo_feat,
                            GUM_sigma, 
                            batch_size,
                            topo_channels, 
                            dis_mat,
                            ...
                            M_idx_4=M_idx_4,
                            scope='nodeshuffle',
                            feature_indices_1=feature_indices_1,
                            feature_indices_2=feature_indices_2,
                            feature_indices_3=feature_indices_3,
                            feature_indices_4=feature_indices_4,
...
def feature_extractor(inputs,
    ...
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        ...
        hidden_features, idx0 = densegcn(l0_features, growth_rate=growth_rate,
                                         n_layers=1, k=k4, d=1,
                                         scope='head1', return_idx=True,
                                         is_training=is_training, use_bn=use_bn, use_ibn=use_ibn, bn_decay=bn_decay)
        if M_features is not None:
            hidden_features = feature_aggregate(hidden_features, M_features, feature_indices)
        feat_fuse = 0.
