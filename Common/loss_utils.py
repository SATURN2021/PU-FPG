...
def pc_distance(pcd1, pcd2, dis_type='emd', radius=1, threshold=0.):
    if dis_type == 'cd':
        return chamfer(pcd1, pcd2, radius=radius, threshold=threshold)
    else:
...
def chamfer(pcd1, pcd2, radius=1.0, threshold=None):
    ...
    cd = 0.5 * dists_forward + 0.5 * dists_backward
    cd = tf.reduce_mean(cd, axis=1)
    cd_norm = cd / radius
    cd_loss = tf.reduce_mean(cd_norm)
    return cd_loss
