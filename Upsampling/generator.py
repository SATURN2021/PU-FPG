...
from feature_points.neighbor_judge import neighbor_judge
from feature_points.normal_judge import normal_judge
from feature_points.sieve import sieve
from tf_lib.gcn_lib.vertex import densegcn
from tf_lib.gcn_lib.edge import knn_M
...
class PUFPG(object):
    ...
    def __call__(self, inputs, normals):
        with tf.variable_scope(self.name, reuse=self.reuse):
            inputs = tf.cast(inputs, dtype=tf.float32)
            normals = tf.cast(normals, dtype=tf.float32)
            if self.opts.use_FP:
                ND = neighbor_judge(inputs, self.opts.k0)
                NC = normal_judge(inputs, normals, self.opts.k2)
                A = ND + NC
                feature_points, feature_indices = sieve(inputs, A, self.opts.FP_rate)    
                feature_points = tf.expand_dims(feature_points, axis=2)
                M_features, M_idx = densegcn(feature_points, 
                                             growth_rate=self.opts.channels,
                                             n_layers=1, 
                                             k=self.opts.k3, 
                                             d=1,
                                             scope='head0',
                ...
                M_idx = knn_M(feature_points, inputs, k=self.opts.k3)
            ...
            features, idx = ops.feature_extractor(inputs,    # idx = k4
                                                  self.opts.block, 
                                                  self.opts.n_blocks,
                                                  self.opts.channels, 
                                                  self.opts.k, 
                                                  ...
                                                  is_training=self.is_training,
                                                  bn_decay=None, M_features=M_features, 
                                                  feature_indices=feature_indices, M_idx=M_idx)
            
            if self.opts.use_spectrum:
                points, indices = sieve(inputs, A, 1)
                points_1 = points[:, :64, :]
                points_2 = points[:, 64:128, :]
                points_3 = points[:, 128:192, :]
                points_4 = points[:, 192:, :]
                indices_1 = indices[:, :64]
                indices_2 = indices[:, 64:128]
                indices_3 = indices[:, 128:192]
                indices_4 = indices[:, 192:]
                M_idx_1 = knn_M(points_1, inputs, k=8)
                M_idx_2 = knn_M(points_2, inputs, k=12)
                M_idx_3 = knn_M(points_3, inputs, k=16)
                M_idx_4 = knn_M(points_4, inputs, k=20)
                A_1 = get_adjacency_matrix_from_MLknn(point_num=256, 
                                                      feature_indices=indices_1, 
                                                      M_idx=M_idx_1, 
                                                      k3=8)
                A_2 = get_adjacency_matrix_from_MLknn(point_num=256, 
                                                      feature_indices=indices_2, 
                                                      M_idx=M_idx_2, 
                                                      k3=12)
                A_3 = get_adjacency_matrix_from_MLknn(point_num=256, 
                                                      feature_indices=indices_3, 
                                                      M_idx=M_idx_3, 
                                                      k3=16)
                A_4 = get_adjacency_matrix_from_MLknn(point_num=256, 
                                                      feature_indices=indices_4, 
                                                      M_idx=M_idx_4, 
                                                      k3=20)
                adjacency_matrix = A_1 + A_2 + A_3 + A_4

                DGG = get_adjacency_matrix_from_knn(inputs, 
                                                    k4=self.opts.k4, 
                                                    ...
                                                    fp_num=64, 
                                                    self_loop=False)
                DGG = adjacency_matrix + DGG
                DGG = adjacency_normalization(DGG)

                topo_feat = DGCN(DGG, 
                                 inputs=inputs, 
                                 batch_size=self.opts.batch_size, 
                                 num_out_channels=self.opts.topo_channels, 
                                 scope='DGCN_1', 
                                 is_training=self.is_training)
                topo_feat = DGCN(DGG, 
                                 inputs=topo_feat, 
                ...
                H = ops.up_unit(x=features, 
                                DGG=DGG, 
                                topo_feat=topo_feat,
                                topo_channels=self.opts.topo_channels,
                                up_ratio=self.up_ratio_real,
                                upsample=self.opts.upsampler,
                                N_idx=idx,
                                M_idx_1=M_idx_1,
                                M_idx_2=M_idx_2,
                                M_idx_3=M_idx_3,
                                ...
                                scope="up_block",
                                use_att=self.opts.use_att,
                                feature_indices_1=indices_1,
                                feature_indices_2=indices_2,
                                feature_indices_3=indices_3,
                                feature_indices_4=indices_4,
                                is_training=self.is_training, 
                                bn_decay=None)
            else:
                H = ops.up_unit(x=features, 
